...
// Horizon display moved to a separate file Horizon.cpp
...
bool IpsDisplay::drawCompass(int16_t x, int16_t y, bool _dirty, bool compass_dirty) {
	...
	if( (wind_display.get() & WD_DIGITS) || (wind_display.get() & WD_ARROW) ){
		...
		else if( wind_enable.get() != WA_OFF ){  // dynamically change type depending on younger calculation
			int wds, wdc;
			float ws, wc;
			bool oks, okc;
			oks = theWind.getWind( &wds, &ws, &ageStraight );
			okc = CircleWind::getWind( &wdc, &wc, &ageCircling);
			if( oks && (!okc || ageStraight <= ageCircling )) {
				wind = ws;
				winddir = wds;
				type = '|';
				wind_ok = true;
			} else if( okc && (!oks || ageCircling <= ageStraight )) {
				wind = wc;
				winddir = wdc;
				type = '/';
				wind_ok = true;
