...
const char * SetupMenuSelectCodes::getEntry() const
{
	if(_entries)
...
void SetupMenuSelectCodes::updateEntry( const char * ent, int sel, int code ) {
...
SetupMenuSelectCodes::SetupMenuSelectCodes( const char* title, e_restart_mode_t restart,
    int (*action)(SetupMenuSelectCodes *p), bool save, SetupNG<int> *anvs,
...
SetupMenuSelectCodes::~SetupMenuSelectCodes()
{
	detach(this);
}
...
void SetupMenuSelectCodes::display( int mode ){
...
void SetupMenuSelectCodes::down(int count){
	if( (selected != this) || !gflags.inSetup )
		return;
	if (_entries == 0)
		return;
...
void SetupMenuSelectCodes::up(int count){
	if( (selected != this) || !gflags.inSetup )
		return;
	if (_entries == 0)
		return;
...
void SetupMenuSelectCodes::longPress(){
	press();
}

void SetupMenuSelectCodes::press(){
	if( (selected != this) || !gflags.inSetup  )
		return;
	if (_entries == 0)
		return;
...
void SetupMenuSelectCodes::delLastEntry() {
	if (_entries == 0)
		return;
	_labels.pop_back();
	_codes.pop_back();
...
bool SetupMenuSelectCodes::existsEntry( std::string ent ){
