...
bool CompassMenu::showSensorRawData(SetupMenuSelect *p)
{
	...
	p->ucg->setPrintPos( 1, 150 );
	float t = sqrt( compass->curX()*compass->curX() + compass->curY()*compass->curY() + compass->curZ()*compass->curZ() )*(1.0f/150.0f);
	if( abs(t-tesla) > 5 )
		tesla += (t-tesla)*0.2;
	...
	if( compass_calibrated.get() ){
		...
		float t= sqrt( compass->calX()*compass->calX() + compass->calY()*compass->calY() + compass->calZ()*compass->calZ() )*(1.0f/150.0f);
