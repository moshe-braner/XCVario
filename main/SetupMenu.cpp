...
void SetupMenu::audio_menu_create_equalizer( MenuEntry *top ){
	...
	SetupMenuValFloat * frqr = new SetupMenuValFloat( PROGMEM"Frequency Response", "%",
...
void SetupMenu::audio_menu_create_volume( MenuEntry *top ){
	...
	SetupMenuValFloat * dv = new SetupMenuValFloat( PROGMEM"Default Volume", "%",
...
void SetupMenu::options_menu_create_compasswind_straightwind_limits( MenuEntry *top ){
	...
	top->addEntry( smslip );
	smslip->setHelp(PROGMEM "Maximum side slip estimation in ° accepted for straight wind calculation");
...
void SetupMenu::system_menu_create_hardware_type( MenuEntry *top ){
	...
	top->addEntry( diso );
#if defined(SUNTON28)
	diso->setHelp( PROGMEM"Display Orientation.  NORMAL means USB jack at bottom, TOPDOWN means USB at top (reboots)");
#else
...
void SetupMenu::system_menu_create_hardware( MenuEntry *top ){
	...
	met_adj->setHelp(PROGMEM"Option to factory factory fine-adjust voltmeter");
...
void SetupMenu::setup_create_root(MenuEntry *top ){
	...
	if( rot_default.get() == 0 ) {
		SetupMenuValFloat * mc = new SetupMenuValFloat( PROGMEM"MC", "",	0.0, 9.9, 0.1, 0, true, &MC );
		mc->setHelp(PROGMEM"MacCready value for optimum cruise speed or average climb rate, in same unit as the variometer");
		mc->setPrecision(1);
		top->addEntry( mc );
	}
	else {
