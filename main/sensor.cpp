	...
	while (1) {
		...
		if( gflags.inSetup != true ) {
			...
			if( active_screen == SCREEN_HORIZON ) {
				if( !(gflags.stall_warning_active || gflags.flarmVisual || gflags.gLoadDisplay) ) {
...
void system_startup(void *args){
	...
	if( err == ESP_OK ){
		...
		// hypot((float)accelG[0], (float)accelG[1], (float)accelG[2])  not supported in current compiler
...
//	ESP_LOGI(FNAME,"Now start T sensor test");
	...
	if (i2c_pins.get() != I2C_NONE) {
		...
		if (i2c_pins.get() == I2C_21)
			i2c.begin(GPIO_NUM_21, GPIO_NUM_22, 100000 );
		else if (i2c_pins.get() == I2C_27)
			i2c.begin(GPIO_NUM_27, GPIO_NUM_22, 100000 );
		bool baok = bmpBA->begin();
		if( baok ){
			ESP_LOGI(FNAME,"BMP280 detected");
			float ba_t, ba_p;
